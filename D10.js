...
const addSkills = () => (me.skills = ["Python", "Javascript", "HTML", "CSS"]);
addSkills();
console.log(me);
...
const addElement = (obj) => me.skills.push(obj);
addElement("Paperino");
console.log(me);
...
*/
const removeElement = () => me.skills.pop();
removeElement();
console.log(me);
...
const dice = () => console.log(Math.ceil(Math.random() * 6));
dice();
...
console.log(whoIsBigger(180, 90));
console.log(whoIsBigger(65, 56));
...
const rollTheDices = (rolls) => {
  ...
  for (let i = 0; i < rolls; i++) {
    const newRolledNum = values.push(dice());
    sum += newRolledNum;
  }
  return { sum, values };
};
...
const isTodayMyBirthday = (date) => {
  ...
  return (
    ...
    currentDate.getDay() === currentDate.getDay()
  );
};

console.log(isTodayMyBirthday("2023-03-15"));
...
const deleteProp = (obj, str) => (delete obj[str], obj);

//utilizzo l'oggetto creato nell'esercizio C
console.log(me);
...
const newestMovie = () => {
  if (movies.length === 0) {
    return "inserisci dei film";
  }

  let positionZero = movies[0];
  for (let i = 1; i < movies.length; i++) {
    const controlMovie = movies[i];
    if (parseInt(controlMovie.Year) > parseInt(positionZero.Year))
      positionZero = controlMovie;
  }
  return positionZero;
};
...
const onlyInLastMillennium = () =>
  movies.filter((film) => parseInt(film.Year) < 2000);
...
const sumAllTheYears = () =>
  movies.reduce((acc, film) => parseInt(film.Year) + acc, 0);
...
console.log(searchByTitle("flies"));
console.log(searchByTitle("gabibbo"));
...
console.log(searchAndDivide("gabibbo"));
...
const tDsText = () => allTDs.forEach((td) => console.log(td.textContent));
tDsText();
